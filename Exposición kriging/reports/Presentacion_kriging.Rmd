...
* Datos de área: $D$ es fijo y aleatorio.
...
* $\mathbf{Z}(\mathbf{s}) = \mu (\mathbf{s}) + e(\mathbf{s})$ con $e(\mathbf{s}) \sim (\mathbf{0}, \boldsymbol{\Sigma})$
* $E[Z(s_i)] = \mu (s_i)$.
* $Var(Z(s_i)) = E[(Z(s_i)-\mu(s_i))^2]=\sigma^2_i$.
* Función de autocovarianza : 

\begin{align}
C(Z(s_i), Z(s_j))&= E[(Z(s_i)-\mu(s_i))(Z(s_j)-\mu(s_j))]
\end{align}
...
$$ Z^*(s_0) = \hat{\mu}+\boldsymbol{\sigma}' \boldsymbol{Z}^{-1}(\mathbf{Z}(\mathbf{s})-\mathbf{1}\hat{\mu})$$
